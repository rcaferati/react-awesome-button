import * as React from 'react';
import type { Meta, StoryObj } from '@storybook/react-webpack5';
import { Shield } from 'lucide-react';

import AwesomeButtonProgress from '../../components/AwesomeButtonProgress';
import defaultStyles from '../../styles/themes/theme-blue';

type AwesomeButtonProgressProps = React.ComponentProps<
  typeof AwesomeButtonProgress
>;

type ShowcaseArgs = {
  type: string;
  disabled: boolean;
  ripple: boolean;
};

const resolvedDefaultStyles =
  (
    defaultStyles as Record<string, string> & {
      default?: Record<string, string>;
    }
  )?.default ?? (defaultStyles as Record<string, string>);

const iconSizeByButtonSize: Record<'small' | 'medium' | 'large', number> = {
  small: 14,
  medium: 16,
  large: 18,
};

const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

function Section({
  title,
  children,
}: {
  title: string;
  children: React.ReactNode;
}): JSX.Element {
  return (
    <section
      style={{
        display: 'grid',
        gap: 12,
        width: 'min(900px, 94vw)',
      }}>
      <h3
        style={{
          margin: 0,
          fontSize: 14,
          fontWeight: 700,
          opacity: 0.85,
          letterSpacing: 0.2,
        }}>
        {title}
      </h3>
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr' }}>
        {children}
      </div>
    </section>
  );
}

function Card({
  label,
  children,
}: {
  label: string;
  children: React.ReactNode;
}): JSX.Element {
  return (
    <div
      style={{
        border: '1px solid #e5e7eb',
        borderRadius: 12,
        padding: 12,
        background: '#fff',
        display: 'grid',
        gap: 10,
      }}>
      <div
        style={{
          fontSize: 12,
          lineHeight: 1.2,
          fontWeight: 600,
          opacity: 0.8,
        }}>
        {label}
      </div>
      <div
        style={{
          minHeight: 56,
          display: 'grid',
          gap: 10,
          justifyItems: 'start',
          alignItems: 'center',
        }}>
        {children}
      </div>
    </div>
  );
}

function ProgressDemo({
  buttonProps,
  iconSize,
  children,
}: {
  buttonProps: Partial<AwesomeButtonProgressProps>;
  iconSize: number;
  children: React.ReactNode;
}): JSX.Element {
  return (
    <AwesomeButtonProgress
      {...(buttonProps as AwesomeButtonProgressProps)}
      before={<Shield size={iconSize} aria-hidden="true" />}
      loadingLabel="Verifying…"
      resultLabel="Verified!"
      releaseDelay={500}
      onPress={async (_event, next) => {
        await sleep(900);
        next(true);
      }}>
      {children}
    </AwesomeButtonProgress>
  );
}

function ShowcaseComponent({
  type,
  disabled,
  ripple,
}: ShowcaseArgs): JSX.Element {
  const makeProps = (
    size: 'small' | 'medium' | 'large' | null
  ): Partial<AwesomeButtonProgressProps> => ({
    size,
    type,
    disabled,
    ripple,
    visible: true,
    between: false,
    placeholder: false,
    moveEvents: true,
    cssModule: resolvedDefaultStyles,
  });

  return (
    <div
      style={{
        display: 'grid',
        gap: 18,
        justifyItems: 'center',
        padding: 8,
        width: 'min(900px, 94vw)',
      }}>
      <Section title="AwesomeButtonProgress · Press to run and complete">
        <Card label="small / medium / large / auto">
          <ProgressDemo
            buttonProps={makeProps('medium')}
            iconSize={iconSizeByButtonSize.medium}>
            Verify
          </ProgressDemo>
          <ProgressDemo
            buttonProps={makeProps('large')}
            iconSize={iconSizeByButtonSize.large}>
            Verify
          </ProgressDemo>

          <ProgressDemo
            buttonProps={makeProps(null)}
            iconSize={iconSizeByButtonSize.medium}>
            Verify signature & continue
          </ProgressDemo>
        </Card>
      </Section>
    </div>
  );
}

const meta: Meta<typeof ShowcaseComponent> = {
  title: 'Icons/AwesomeButtonProgress',
  component: ShowcaseComponent,
  tags: ['autodocs'],
  parameters: {
    layout: 'centered',
  },
  args: {
    type: 'primary',
    disabled: false,
    ripple: false,
  },
  argTypes: {
    type: {
      control: 'text',
      table: { category: 'Button' },
    },
    disabled: {
      control: 'boolean',
      table: { category: 'State' },
    },
    ripple: {
      control: 'boolean',
      table: { category: 'Behavior' },
    },
  },
};

export default meta;

type Story = StoryObj<typeof meta>;

export const Showcase: Story = {};
